...
const urlsToCache = [
  ...
  '/danis/utils/push-config.js',
  '/danis/utils/push-notifications.js'
];
...
self.addEventListener('push', event => {
  ...
  const options = {
    ...
    icon: '/danis/icons/icon-192x192.png',
    badge: '/danis/icons/icon-96x96.png',
    tag: payload.tag || 'rservasroma',
    data: {
      url: payload.url || '/danis/admin.html',
      ...(payload.data || {})
    }
  };
...
self.addEventListener('notificationclick', event => {
  ...
  const targetUrl = event.notification?.data?.url || '/danis/admin.html';
  event.waitUntil(
